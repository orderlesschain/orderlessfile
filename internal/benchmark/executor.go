...
import (
	"context"
	"encoding/csv"
	"gitlab.lrz.de/orderless/orderlessfile/contractsbenchmarks/benchmark/benchmarkfunctions"
	"gitlab.lrz.de/orderless/orderlessfile/internal/benchmark/benchmarkutils"
	"gitlab.lrz.de/orderless/orderlessfile/internal/benchmark/bencmarkdb"
	"gitlab.lrz.de/orderless/orderlessfile/internal/benchmark/latencymeasurment"
...
func MakeNewTransactionResultOrderlessFile(counter, endorsementPolicyWithExtra, endorsementPolicy int, signer *signer.Signer) *TransactionResult {
...
func (t *TransactionResult) PreProcessFileTransaction(proposal *protos.ProposalRequest, startTime time.Time) {
	t.readWriteType = proposal.WriteReadTransaction
	t.fileId = proposal.MethodParams[0]
	if len(proposal.MethodParams) > 1 {
		t.fileVersion = proposal.MethodParams[1]
		t.shardId = proposal.MethodParams[2]
		t.fileDataLength = proposal.MethodParams[3]
		t.clientClock = proposal.MethodParams[4]
		t.upload = true
	} else {
		t.receivedProposalExpected = 1
		t.receivedTransactionCommitExpected = 1
	}
	t.latencyMeasurementInstance = t.latencyMeasurement(startTime)
}

func (t *TransactionResult) PostProcessFileTransaction() {
	t.startTime, t.latency, t.endTime = t.latencyMeasurementInstance()
}
...
type proposalStream struct {
	streamOrderlessFile protos.TransactionService_ProcessProposalOrderlessFileStreamClient
}

type transactionStream struct {
	streamOrderlessFile protos.TransactionService_CommitOrderlessFileTransactionStreamClient
}
...
func NewExecutor() *Executor {
	...
	if config.Config.IsOrderlessFile {
		go tempEx.subscriberForTransactionEventsOrderlessFile()
	...
	if config.Config.IsOrderlessFile {
		go tempEx.subscriberForProposalEventsOrderlessFile()
		go tempEx.makeAllStreamTransactionOrderlessFile()
	}
...
func (rex *RoundExecutor) runExecuteBenchmarkStarter() {
	if config.Config.IsOrderlessFile {
		for {
			...
			go rex.executeTransactionPart1OrderlessFile(starter.counter, starter.startTime)
		}
	}
}
...
func (ex *Executor) setInExperimentParticipatingFaultyComponents(nodes *protos.FaultyNodes) {
	...
	go ex.subscriberForNewlyAddedProposalEventsOrderlessFile(newlySelectedOrgsEndorsementPolicy)
	go ex.subscriberForNewlyAddedTransactionEventsOrderlessFile(newlySelectedOrgsEndorsementPolicy)

	for nodeId, _ := range newlySelectedOrgsEndorsementPolicy {
		if err := ex.makeSingleStreamProposal(nodeId); err != nil {
			logger.ErrorLogger.Println(err)
		}
		if err := ex.makeSingleStreamTransactionOrderlessFile(nodeId); err != nil {
			logger.ErrorLogger.Println(err)
		}
	}
...
func (rex *RoundExecutor) ExportReportFile() {
	reportPath := filepath.Join("./orderlessfile-experiments/results/",
...
func (ex *Executor) makeSingleStreamProposal(node string) error {
	...
	if config.Config.IsOrderlessFile {
		tempStream.streamOrderlessFile, err = client.ProcessProposalOrderlessFileStream(context.Background())
	}
	if err != nil {
