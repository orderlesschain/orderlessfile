...
func (m *Manager) modifyMVRegisterFile(leafNode *protos.CRDTObjectNode, operation *protos.CRDTOperation) {
...
func (m *Manager) modifyLeafNode(leafNode *protos.CRDTObjectNode, operation *protos.CRDTOperation) {
	...
	case protos.CRDTOperation_FILE_SHARD:
		m.modifyMVRegisterFile(leafNode, operation)
...
func (m *Manager) applyFileOperationsToObject(crdtObj *CRDTObject, tx *protos.Transaction) {
	operation := &protos.CRDTOperation{
		ValueType:     protos.CRDTOperation_FILE_SHARD,
		Value:         tx.ShardData,
		OperationPath: []string{tx.ShardId, tx.FileVersion},
		Clock:         tx.ClientClock,
	}
	crdtObj.Lock.Lock()
...
func (m *Manager) ApplyFileOperationsWarm(tx *protos.Transaction) {
	crdtObjId := tx.ContractName + tx.FileID
	m.lock.RLock()
	crdtObj, ok := m.crdtObjs[crdtObjId]
	m.lock.RUnlock()
	if !ok {
		crdtObj = m.newCRDTObject(crdtObjId)
		m.lock.Lock()
		m.crdtObjs[crdtObjId] = crdtObj
		m.lock.Unlock()
	}
	m.applyFileOperationsToObject(crdtObj, tx)
}
